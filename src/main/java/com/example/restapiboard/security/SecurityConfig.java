...
import com.example.restapiboard.config.HTMLCharacterEscapes;
...
/*
        http.authorizeRequests()
                ...
                // css 폴더를 login 없이 허용
                .antMatchers("/css/**").permitAll()     //css로 시작하는 모든 부분들은 인증없이 접근가능
                .antMatchers("/user/**").permitAll()    //user로 시작하는 모든 부분들은 인증없이 접근가능
                .antMatchers("/h2-console/**").permitAll()    //db도 인증없이 접근가능
                .anyRequest().authenticated()   //모든 요청이 인증이 필요함(css 파일도 인증이 필요함)
                ...
                .and()
                .logout()
                .logoutUrl("/user/logout")  //로그아웃 컨트롤러를 따로 만들 필요 없이 이 url요청이 들어오면 로그아웃 해준다
                .permitAll()
